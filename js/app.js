...
const dom = {
    pages: {
        ...
        business: document.getElementById('page-business')
    },
...
function setupForms() {
    ...
    dom.businessForm.addEventListener('submit', (e) => {
        e.preventDefault();
        saveSettings();
    });
...
// Input Validation & Auto-format
function setupInputValidation() {
    // Sort Code - auto-format with dashes
    ...
    // Phone - allow +, spaces, parentheses, numbers
    ...
    // Companies House - alphanumeric
    const companiesInput = document.getElementById('companiesHouse');
    if (companiesInput) {
        companiesInput.addEventListener('input', (e) => {
...
function addLineItem() {
    ...
    div.innerHTML = `
        <div class="line-item-header">
            <input type="text" placeholder="Item description" class="item-desc" maxlength="100">
            <button type="button" class="remove-btn">&times;</button>
        </div>
        <div class="line-item-row">
            <input type="number" placeholder="Qty" value="1" min="1" class="item-qty" inputmode="numeric">
            <input type="number" placeholder="Price" value="0" min="0" step="0.01" class="item-price" inputmode="decimal">
...
function getLineItems() {
    const items = [];
    dom.lineItemsContainer.querySelectorAll('.line-item').forEach(row => {
            ...
            <table class="invoice-detail-table">
                ...
                <tbody>
                    ${invoice.lineItems.map(item => `
                        <tr>
                            <td title="${escapeHtml(item.description)}">${truncateText(item.description, 40)}</td>
                            <td>${item.quantity}</td>
...
function downloadPDF() {
    ...
    // Columns: Desc (70), Qty (20), Rate (40), Amount (40)
    const colDesc = margin + 2;
    const colQty = margin + 75;
    const colRate = margin + 100;
    const colAmount = pageWidth - margin - 25;
    
    doc.text('DESCRIPTION', colDesc, tableTop + 7);
    doc.text('QTY', colQty, tableTop + 7);
    doc.text('RATE', colRate, tableTop + 7);
    doc.text('AMOUNT', colAmount, tableTop + 7, { align: 'right' });
    ...
    currentInvoice.lineItems.forEach((item, idx) => {
        ...
        const maxDescLen = 45;
        const desc = item.description.length > maxDescLen ? item.description.substring(0, maxDescLen) + '...' : item.description;
        doc.text(desc, colDesc, y + 2);
        doc.text(item.quantity.toString(), colQty, y + 2);
        doc.text(formatCurrency(item.unitPrice), colRate, y + 2);
        doc.text(formatCurrency(item.quantity * item.unitPrice), colAmount, y + 2, { align: 'right' });
        y += 10;
    });
    ...
    if (settings.sortCode && settings.accountNumber) {
        ...
        y += 6;
        doc.text(`Sort Code: ${settings.sortCode}  |  Account: ${settings.accountNumber}`, margin, y);
    }
